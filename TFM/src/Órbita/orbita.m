...
% Debemos meter otra muestra si no coinciden, ya que la función states lo tiene en cuenta.
if tiempos(end) ~= escenario.StopTime  
    tiempos(end+1) = escenario.StopTime;
end
...
T_MOT = table(segundos, MOT, veh_mot, pos_ecef(:,1), pos_ecef(:,2), pos_ecef(:,3), ...
    vel_ecef(:,1), vel_ecef(:,2), vel_ecef(:,3), ace_ecef(:,1), ace_ecef(:,2), ace_ecef(:,3), ...
